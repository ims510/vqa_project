...
class CoAttentionMCQNet(nn.Module):
    def __init__(self, hidden_dim=512):
        super(CoAttentionMCQNet, self).__init__()
        

        # # Define the weight matrix as a learnable parameter
        # self.weight_matrix = nn.Parameter(torch.randn(batch_size, batch_size))

        # # Attention transformation layers
        # self.Wv = nn.Linear(batch_size, hidden_dim)
        # self.Wq = nn.Linear(batch_size, hidden_dim)
...
class CustomDataLoader:
    ...
    #     image_features = np.array([np.array(eval(x)) if isinstance(x, str) else np.array(x) for x in data['feature_vector'].values])
        
    #     # Extract question embeddings
    #     question_features = np.array([np.array(eval(x)) if isinstance(x, str) else np.array(x) for x in data['question_embedding'].values])
        
    #     # Extract all answer choices
    #     choice_columns = ['choice1_embedding', 'choice2_embedding', 'choice3_embedding', 'choice4_embedding']
    #     answer_choices = np.stack(
    #         [np.array([np.array(eval(x)) if isinstance(x, str) else np.array(x) for x in data[col].values]) for col in choice_columns],
    #         axis=1
    #     )  # Shape: (N, 4, 768)

    #     # Convert to PyTorch tensors
    #     image_features = torch.tensor(image_features).float()
    #     question_features = torch.tensor(question_features).float()
    #     answer_choices = torch.tensor(answer_choices).float()

    #     # Extract labels
    #     labels = torch.tensor(np.array(data['answer'].tolist()).argmax(axis=1)).long()

    #     dataset = TensorDataset(image_features, question_features, answer_choices, labels)
