...
def inspect_and_clean_dataframe(df):
    ...
    for column in df.columns:
        ...
        if (
            column.startswith("choice")
            and "embedding" in column
            or column in ["feature_vector", "question_embedding"]
        ):
...
df_with_features = add_feature_vectors_to_df(
    df, device
)  # add image embeddings using resnet50
df_with_embeddings = add_text_embeddings_to_df(
    df_with_features, device
)  # add text embeddings using bert
